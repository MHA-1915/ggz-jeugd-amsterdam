...
// Exacte coördinaten voor OKT adressen (opgezocht via Google Maps)
...
Object.keys(exactCoordinates).forEach(adres => {
    const coords = exactCoordinates[adres];
    
    // Zoek naar het adres en vervang de coördinaten
    const regex = new RegExp(`(adres: "${adres.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}",[^}]*positie: \\{ lat: )[^,]+(, lng: )[^}]+( \\})`, 'g');
    const replacement = `$1${coords.lat}$2${coords.lng}$3`;
    
    content = content.replace(regex, replacement);
    console.log(`Bijgewerkt: ${adres} -> ${coords.lat}, ${coords.lng}`);
});

// Schrijf het bestand terug
fs.writeFileSync('GGZjeugdAmsterdam.js', content, 'utf8');

console.log('\nAlle OKT coördinaten zijn bijgewerkt!'); 